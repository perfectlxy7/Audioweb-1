					...
					<li class=""  id="lm${menu1.mid }">
						...
						<c:forEach items="${menu1.subMenu}" var="menu2">
							...
							<li class="" id="z${menu2.mid }">
								<a style="cursor:pointer;" <c:if test="${not empty menu2.murl && '#' != menu2.murl}">target="mainFrame" onclick="siMenu('z${menu2.mid }','lm${menu1.mid }','${menu2.mname }','${menu2.murl }','${menu2.refresh}')"</c:if><c:if test="${'[]' != menu2.subMenu}"> class="dropdown-toggle"</c:if>>
								...
								<ul class="submenu">
									...
									<c:if test="${menu3.hasMenu}">
										<li class="" id="m${menu3.mid }">
											<a style="cursor:pointer;" <c:if test="${not empty menu3.murl && '#' != menu3.murl}">target="mainFrame" onclick="siMenu('m${menu3.mid }','z${menu2.mid }','${menu3.mname }','${menu3.murl }','${menu3.refresh}')"</c:if><c:if test="${'[]' != menu3.subMenu}"> class="dropdown-toggle"</c:if>>
											...
											<ul class="submenu">
												...
												<li class="" id="n${menu4.mid }">
													...
													<c:if test="${'[]' == menu4.subMenu}">
													<a style="cursor:pointer;" target="mainFrame" <c:if test="${not empty menu4.murl && '#' != menu4.murl}">target="mainFrame" onclick="siMenu('n${menu4.mid }','m${menu3.mid }','${menu4.mname }','${menu4.murl }','${menu4.refresh }')"</c:if>>
													</c:if>
