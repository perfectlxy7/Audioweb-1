...
Dialog.prototype.create = function () {
    ...
    if (!this.Width) this.Width = Math.round(bd.clientWidth * 4 / 10);//默认width
    else if(parseFloat(this.Width)<1)//设置的百分比小于1，如0.6
    	this.Width = Math.round(bd.clientWidth * this.Width);
    
    if (!this.Height) this.Height = Math.round(this.Width / 2);
    else if(parseFloat(this.Height)<1)
    	this.Height = Math.round(bd.clientHeight * this.Height);
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td id="_ResizeTR_' + this.ID + '" width="13" height="33" style="background-image: url(' + IMAGESPATH + 'dialog_rt.png) !important;background: url(' + IMAGESPATH + 'dialog_rt.gif) no-repeat right 0;"><div style="width: 13px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td id="_ResizeL_' + this.ID + '" width="13" style="background-image: url(' + IMAGESPATH + 'dialog_mlm.png) !important;background: url(' + IMAGESPATH + 'dialog_mlm.gif) repeat-y left;"></td>\
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          ...
          <tr>\
            <td valign="top" align="center"><div id="_Container_' + this.ID + '" style="position: relative;background-color:#fff; width: ' + this.Width + 'px; height: ' + this.Height + 'px;background-color: #fff;">\
	...
	' + (function (obj) {
      ...
      <td id="_ResizeR_' + this.ID + '" width="13" style="background-image: url(' + IMAGESPATH + 'dialog_mrm.png) !important;background: url(' + IMAGESPATH + 'dialog_mrm.gif) repeat-y right;"></td>\
    </tr>\
    <tr>\
      <td id="_ResizeLB_' + this.ID + '" width="13" height="13" style="background-image: url(' + IMAGESPATH + 'dialog_lb.png) !important;background: url(' + IMAGESPATH + 'dialog_lb.gif) no-repeat 0 bottom;"></td>\
      <td id="_ResizeB_' + this.ID + '" style="background-image: url(' + IMAGESPATH + 'dialog_cb.png) !important;background: url(' + IMAGESPATH + 'dialog_cb.gif) repeat-x bottom;"></td>\
      ...
      '+(this.Resize ? '<div id="_ResizeBR_' + this.ID + '" style="position:absolute; width:14px; height:14px; right:10px; bottom:10px; overflow:hidden; cursor:nw-resize; background-image: url(' + IMAGESPATH + 'resize.png);"></div>' : '')+'\
</div>\
'
...
Dialog.prototype.show = function () {
    ...
    this.opened = true;
	bgdiv=null;
	containerDiv = this.getContainerDiv();
	if(this.Resize && containerDiv){
		if(this.getResizeDiv("BR")) this.resize(containerDiv,this.getResizeDiv("BR"));
	}
};
...
改变大小函数
+-------------------------- */
Dialog.prototype.resize = function (oParent,handle)
{
	var thisdialog = this;
	handle.onmousedown = function (event)
	{
		...
		document.onmousemove = function (event)
		{
			...
			var iW = handle.offsetWidth + iL;
			var iH = handle.offsetHeight + iT;

			iW < dragMinWidth && (iW = dragMinWidth);
			iW > maxW && (iW = maxW);
			topWin.$id("_DialogTable_" + thisdialog.ID).width = iW+10;
			oParent.style.width = iW -16+ "px";

			iH < dragMinHeight && (iH = dragMinHeight);
			iH > maxH && (iH = maxH);
			oParent.style.height = iH -36+ "px";
			return false;	
		};
...
Dialog.prototype.getContainerDiv = function () {
	var containerDiv=topWin.$id("_Container_" + this.ID)
...
Dialog.prototype.getResizeDiv = function (direction ) {
	var resizeDiv=topWin.$id("_Resize"+direction+"_" + this.ID)
	if(!resizeDiv) return null;
	try{
		return resizeDiv;
	}finally{
		resizeDiv = null;
...
Dialog.alert = function (msg,btnname, func, w, h) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt;padding:10px;">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.style.display = "none";
    if(btnname==null || btnname=="")   diag.cancelButton.value = "确 定"; else diag.cancelButton.value = btnname;
